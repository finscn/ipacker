...
(function(exports) {
    ...
    Packer.prototype.expandFreeRectangles = function(width, height, packRule) {
        ...
        // if (this.square) {
        //     expandX = Math.max(expandX, expandY);
        //     expandY = expandX;
        // }
    ...
    var PrepareRule = {
        ...
        'default': function(rectangles, sortRule, packer) {
            ...
            var fixMaxWidth = Math.floor(packer.maxWidth / 2);
            var fixMaxHeight = Math.floor(packer.maxHeight / 2);
            ...
            if (packer.square) {
                ...
                estimateWidth = Math.min(hySide, fixMaxWidth, fixMaxHeight);
                estimateHeight = estimateWidth;
            } else {
                // TODO

                // estimateWidth = Math.ceil(info.totalWidth / info.count * 4);
                // estimateHeight = Math.ceil(info.totalArea / estimateWidth);

                estimateWidth = Math.min(hyWidth, fixMaxWidth);
                estimateHeight = Math.min(hyHeight, fixMaxHeight);
                ...
                //     estimateWidth = Math.ceil(info.totalArea / estimateHeight);
                // }
            }
