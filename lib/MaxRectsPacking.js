...
(function(exports) {
     ...
     *        expandStepX: expand-freeSpce-step in x-axis
     *        expandStepY: expand-freeSpce-step in y-axis
     */
    ...
    Packer.prototype.expandFreeRectangles = function(width, height, packRule) {
        ...
        var expandX = this.expandStepX;
        var expandY = this.expandStepY;

        var fixWidth = Math.floor(width * 3 / 4);
        var fixHeight = Math.floor(height * 3 / 4);

        var minSide = Math.min(fixWidth, fixHeight);
        var maxSide = Math.max(fixWidth, fixHeight);

        expandX = expandX || fixWidth;
        expandY = expandY || fixHeight;

        if (this.square) {
            minSide = Math.min(expandX, expandY);
            expandX = Math.max(30, Math.floor(minSide * 3 / 4));
            expandY = expandX;
        }

        var newRight = Math.min(this.maxWidth, this.right + expandX);
        var dx = newRight - this.right;

        var newBottom = Math.min(this.maxHeight, this.bottom + expandY);
        var dy = newBottom - this.bottom;

        var expand = false;

        if (this.freeRectangles.length > 0 && (dx > 0 || dy > 0)) {
        ...
        if (!expand) {
            if (dx > 0) {
                this.freeRectangles.push(new Rect(this.right, 0, dx, newBottom))
                expand = true;
            }
            if (dy > 0) {
                this.freeRectangles.push(new Rect(0, this.bottom, newRight, dy))
                expand = true;
            }
        }
    ...
    var PrepareRule = {
        ...
        'default': function(rectangles, sortRule, packer) {
            ...
            if (packer.square) {
                // TODO
                estimateWidth = (info.totalWidth + info.totalHeight) / 2 / info.count >> 0;
                estimateHeight = estimateWidth;
            } else {
                // TODO
                var fixMaxWidth = Math.floor(packer.maxWidth * 2 / 4);
                var fixMaxHeight = Math.floor(packer.maxHeight * 2 / 4);

                if (info.totalWidth >= info.totalHeight) {
                    estimateWidth = fixMaxWidth;
                    estimateHeight = Math.min(fixMaxHeight, Math.ceil(info.totalArea / estimateWidth));
                } else {
                    estimateHeight = fixMaxHeight;
                    estimateWidth = Math.min(fixMaxWidth, Math.ceil(info.totalArea / estimateHeight));
                }
            }

            // var es = Math.ceil(Math.sqrt(info.totalArea));
            // es = 2 ** Math.ceil(Math.log(es) * Math.LOG2E);
            // estimateWidth = Math.min(packer.maxWidth, es);
            // estimateHeight = estimateWidth;

            packer.right = packer.freeSpaceWidth || estimateWidth;
            packer.bottom = packer.freeSpaceHeight || estimateHeight;

            // console.log((info.minWidth + info.maxWidth) / 2, (info.minHeight + info.maxHeight) / 2)
            // console.log(info.totalWidth / info.count >> 0, info.totalHeight / info.count >> 0)
            // console.log('estimate -', estimateWidth, estimateHeight);
